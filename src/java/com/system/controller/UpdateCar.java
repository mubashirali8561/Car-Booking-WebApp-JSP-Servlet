...
public class UpdateCar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Car car = new Car(cid,title,discription,rent,mileage,transmission,seats,luggage,fuel,Airconditions,ChildSeat,GPS,Luggage_tick,Music,SeatBelt,SleepingBed,Water,Bluetooth,Onboardcomputer,Audioinput,Long_Term_Trips,CarKit,Remote_central_locking,Climatecontrol);
            CarDao dao = new CarDao(ConnectionProvider.getConnection());
            
            if(dao.updateCar(car))
            {
                out.println("done");
                Massage m = new Massage("Car Details Update Successfully.","success","alert-success");
                s.setAttribute("msg",m);
                response.sendRedirect("admin/dashboard.jsp");
             }else
             {
                out.println("Error! While Posting Comment.");
                Massage m = new Massage("Error! While Updating Car Details.","error","alert-danger");
                s.setAttribute("msg",m);
                response.sendRedirect("admin/dashboard.jsp");
             }
        }
